...
class ClientReceiptAuth extends StatefulWidget {
  ...
  @override
  State<ClientReceiptAuth> createState() =>
      _ClientReceiptAuthState(business_num, date, menu, a_prize, count, total);
}

class _ClientReceiptAuthState extends State<ClientReceiptAuth> {
  var business_num, date, menu, a_prize, count, total;

  _ClientReceiptAuthState(this.business_num, this.date, this.menu, this.a_prize,
  ...
  int _saveIndex = 0;
  var tmpImg =
      'https://firebasestorage.googleapis.com/v0/b/chipin-c3559.appspot.com/o/support%2F%EC%B4%88%EB%A1%9D%EC%9A%B0%EC%82%B0.png?alt=media&token=d0a55dba-e01c-421c-a314-25dde8ffbd0c';
    ...
    return Scaffold(
        ...
        body: FutureBuilder(
            ...
            builder: (context, snapshot) {
              if (!snapshot.hasData) {
                return Center(
                  child: Text('해당 영수증과 일치하는 가맹점이 없습니다.',
                      style: TextStyle(
  ...
  writeClientHistory(year, month, day, hour, minute, title, point) {
    FirebaseFirestore.instance.collection('History').doc(userid).update({
